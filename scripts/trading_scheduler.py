...
def place_order(symbol, order_type, quantity, side="BUY", price=None, time_in_force="DAY",
                use_bracket_order=False, take_profit_pct=None, stop_loss_pct=None):
    """
    ...
    try:
        ...
        if use_bracket_order:
            if take_profit_pct is not None:
                ...
                take_profit_price = price * (1 + take_profit_pct/100) if side.upper() == "BUY" else price * (1 - take_profit_pct/100)
                order_data["take_profit"] = {"limit_price": str(round(take_profit_price, 2))}
                
            if stop_loss_pct is not None:
                # Calculate stop loss price
                stop_loss_price = price * (1 - stop_loss_pct/100) if side.upper() == "BUY" else price * (1 + stop_loss_pct/100)
        ...
        api_endpoint = f"{TRADING_SYSTEM_PATH}/api/orders"
